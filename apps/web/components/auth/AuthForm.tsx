...
export function AuthForm() {
  ...
  return (
    <main className="grid min-h-[calc(100vh-5rem)] w-full lg:grid-cols-2">
      {/* Panel de marca (solo desktop) */}
      <aside className="relative hidden overflow-hidden bg-[#06162b] lg:flex lg:flex-col lg:justify-between lg:p-12">
          ...
          <ul className="mt-10 space-y-5">
            {highlights.map(({ Icon, title, text }) => (
              <li key={title} className="flex gap-3">
                <span className="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 text-blue-200">
        ...
        <div className="relative z-10 overflow-hidden rounded-xl border border-white/10 bg-white/5">
          {/* eslint-disable-next-line @next/next/no-img-element -- mock de producto entregado por el equipo */}
          <img src="/brand/mock-inicio.png" alt="Vista de El Compañero" className="w-full object-cover" />
        </div>
      </aside>

      {/* Formulario */}
      <section className="flex flex-col justify-center bg-white px-5 py-12 sm:px-10">
